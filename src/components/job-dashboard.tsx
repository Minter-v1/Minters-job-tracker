...
import { CalendarIcon, CloseIcon, ExternalIcon, PlusIcon, SearchIcon, TrashIcon } from "@/components/icons";
...
export function JobDashboard({ userEmail }: { userEmail: string }) {
  ...
  return <div className="min-h-screen bg-[#f6f7f9] text-slate-900">
    ...
    <main className="mx-auto max-w-[1320px] px-5 py-8 sm:px-8 lg:py-10"><div className="mb-7 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h1 className="text-2xl font-bold tracking-[-.035em]">지원 현황</h1><p className="mt-1.5 text-sm text-slate-500">기업별 전형 과정과 일정을 한눈에 관리하세요.</p></div><p className={`text-xs ${saveError ? "text-rose-500" : "text-slate-400"}`}>{saveError || (pendingWrites ? "저장 중..." : "저장됨")}</p></div>
      ...
      {viewMode === "목록" ? <JobList jobs={jobs} visible={visible} filter={filter} setFilter={setFilter} query={query} setQuery={setQuery} onSelect={setSelectedId} onStepChange={(id, step) => updateJob(id, { currentStep: step })}/> : <CalendarView jobs={jobs} month={calendarMonth} setMonth={setCalendarMonth} onSelect={setSelectedId}/>}
    </main>
...
function JobList({ jobs, visible, filter, setFilter, query, setQuery, onSelect, onStepChange }: { jobs: Job[]; visible: Job[]; filter: Filter; setFilter: (filter: Filter) => void; query: string; setQuery: (query: string) => void; onSelect: (id: string) => void; onStepChange: (id: string, step: string) => void }) { return <section className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="flex flex-col gap-3 border-b border-slate-200 p-3 md:flex-row md:items-center md:justify-between"><div className="flex gap-1 overflow-x-auto">{filters.map((item) => <button key={item} onClick={() => setFilter(item)} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-semibold transition-colors ${filter === item ? "bg-cyan-700 text-white shadow-sm" : "text-slate-500 hover:bg-cyan-50 hover:text-cyan-800"}`}>{item}</button>)}</div><label className="relative md:w-72"><SearchIcon className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400"/><input value={query} onChange={(event) => setQuery(event.target.value)} className="plain-field pl-9" placeholder="기업명 또는 직무 검색"/></label></div><div className="hidden grid-cols-[minmax(190px,1.25fr)_140px_minmax(220px,1.3fr)_140px_110px_28px] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-3 text-[10px] font-bold text-slate-400 md:grid"><span>기업 / 직무</span><span>현재 단계</span><span>전형 방식</span><span>예정 일정</span><span>전체 과정</span><span/></div>{visible.length ? <div className="divide-y divide-slate-100">{visible.map((job) => <JobRow key={job.id} job={job} onSelect={() => onSelect(job.id)} onStepChange={(step) => onStepChange(job.id, step)}/>)}</div> : <div className="px-6 py-20 text-center"><p className="font-bold">{jobs.length ? "해당하는 지원이 없습니다." : "아직 등록한 지원이 없습니다."}</p></div>}</section>; }
...
function CalendarEventItem({ event, weekday, onSelect }: { event: CalendarEvent; weekday: number; onSelect: (id: string) => void }) {
  ...
  const periodTone = event.completed ? "bg-slate-200 text-slate-500" : "bg-cyan-100 text-cyan-800 hover:bg-cyan-200";
  const pointTone = event.completed ? "bg-slate-100 text-slate-400 line-through" : event.kind === "deadline" ? "bg-rose-50 text-rose-600" : "bg-cyan-50 text-cyan-700 hover:bg-cyan-100";

  return <button onClick={() => onSelect(event.job.id)} aria-label={`${event.job.company} ${event.title}`} className={`relative z-[1] block min-h-6 truncate px-1.5 py-1 text-left text-[9px] font-semibold transition-colors sm:text-[10px] ${isPeriod ? `${periodTone} ${continuesRight ? "w-[calc(100%+1px)]" : "w-full"}` : `${pointTone} mx-2 w-[calc(100%-1rem)] rounded-md`} ${roundLeft ? "rounded-l-md" : ""} ${roundRight ? "rounded-r-md" : ""}`}>
