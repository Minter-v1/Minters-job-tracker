...
function CalendarView({ jobs, month, setMonth, onSelect }: { jobs: Job[]; month: Date; setMonth: (date: Date) => void; onSelect: (id: string) => void }) {
  ...
  return <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
    ...
    <div className="grid grid-cols-7">{days.map((date) => {
      ...
      return <div key={key} className={`min-h-24 border-b border-r border-slate-100 p-1.5 sm:min-h-32 sm:p-2 ${outside ? "bg-slate-50/60" : ""}`}><div className={`mb-1 flex size-6 items-center justify-center rounded-full text-[10px] font-semibold ${key === today ? "bg-cyan-500 text-white" : outside ? "text-slate-300" : "text-slate-500"}`}>{date.getDate()}</div><div className="space-y-1">{daily.slice(0, 3).map((event) => <CalendarEventItem key={`${event.job.id}-${event.kind}-${event.title}-${event.date}`} event={event} weekday={date.getDay()} onSelect={onSelect}/>)}{daily.length > 3 && <p className="px-1 text-[9px] text-slate-400">+{daily.length - 3}개</p>}</div></div>;
    })}</div>
  </section>;
}
...
function CalendarEventItem({ event, weekday, onSelect }: { event: CalendarEvent; weekday: number; onSelect: (id: string) => void }) {
  ...
  return <button onClick={() => onSelect(event.job.id)} aria-label={`${event.job.company} ${event.title}`} className={`block min-h-6 w-auto truncate px-1.5 py-1 text-left text-[9px] font-semibold transition-colors sm:text-[10px] ${isPeriod ? periodTone : pointTone} ${continuesLeft ? "-ml-1.5 sm:-ml-2" : ""} ${continuesRight ? "-mr-1.5 sm:-mr-2" : ""} ${roundLeft ? "rounded-l-md" : ""} ${roundRight ? "rounded-r-md" : ""}`}>
